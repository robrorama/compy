...
def plot_data(dataframe, ticker, peaks, troughs):
    """
    Plot candlestick chart, volume, MACD, RSI, peak-trough I-bars, recovery boxes, and percentage changes.
    """
    ...
    for (peak_index, peak_price, trough_index, trough_price) in pairs:
        ...
        # Drawdown Percentage
        drawdown_percentage = ((trough_price - peak_price) / peak_price) * 100
        figure.add_annotation(
            x=trough_x,
            y=trough_price,
            text=f"{drawdown_percentage:.1f}%",
            showarrow=False,
    ...
    # Yellow Recovery Boxes and Percentage Increase
    for i in range(len(pairs) - 1):
        ...
        # Recovery Percentage
        recovery_percentage = ((next_peak_price - trough_price) / trough_price) * 100
        figure.add_annotation(
            x=next_peak_x,
            y=next_peak_price,
            text=f"{recovery_percentage:.1f}%",
            showarrow=False,
            font=dict(size=10, color="black"),
            row=1,
            col=1,
