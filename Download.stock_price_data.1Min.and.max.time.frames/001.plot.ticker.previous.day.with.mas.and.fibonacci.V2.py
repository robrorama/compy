...
def plot_ticker_minute_data_with_ma_fib(ticker, days):
    ...
    ax.plot(filtered_data.index, filtered_data['Close'], label='Close Price', color='black', alpha=0.6)
    ax.set_title(f"{ticker.upper()} - {end_date.strftime('%Y-%m-%d')} (Last {days} Day(s))")
    ax.set_xlabel('Date')
    ax.set_ylabel('Price')

    # Plot moving averages
    for ma in [9, 20, 50, 100, 150, 200, 300]:
        if ma in filtered_data.columns:
            ax.plot(filtered_data.index, filtered_data[f'MA{ma}'], label=f'MA{ma}')

    # Plot Fibonacci retracement levels
    for level_name, level_value in levels.items():
        ax.axhline(level_value, linestyle='--', alpha=0.5, color='gray', label=f'Fib {level_name}')
