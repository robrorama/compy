...
for ticker in tickers:
    ...
    data_file_path = f'{dir_path}/{ticker}.data.csv'
    ...
    if not os.path.exists(file_path):
        ...
        # Replace commas with underscores in the data
        data_read = data_read.applymap(lambda x: x.replace(',', '_') if isinstance(x, str) else x)

        # Save the data to a new CSV file with column headers
        data_read.to_csv(data_file_path, index=False)
