...
def calculate_high_volume_days(df):
    ...
    df['ZScore'] = (df['Volume'] - df['RollingAvgVolume']) / df['VolumeStdDev']
    #high_volume_days = df[df['ZScore'] > 2]  # Adjust the Z-score threshold as needed
    volume_threshold = df['RollingAvgVolume'] + (df['VolumeStdDev'] * 2)
    high_volume_days = df[(df['ZScore'] > 2) & (df['Volume'] > volume_threshold)]
    min_marker_size = 25 
    max_marker_size = 300
...
def identify_recurring_high_volume_days(high_volume_days):
    """Identifies recurring high-volume days based on month-day occurrences."""

    # Extract month and day
    high_volume_days['MonthDay'] = high_volume_days.index.map(lambda x: x.strftime('%m-%d'))

    # Count occurrences of each month-day combination
    month_day_counts = Counter(high_volume_days['MonthDay'])

    # Filter for recurring patterns (adjust threshold as needed)
    recurring_days = {day: count for day, count in month_day_counts.items() if count >= 3}  
    return recurring_days


def plot_data(df, high_volume_days, recurring_days, ticker, period_name, start_date):
    ...
    # Add blue "Z" markers for high-volume days
    fig.add_trace(go.Scatter(
        x=high_volume_days.index,
        y=high_volume_days['Adj Close'],
        mode='text',
        text=['Z'] * len(high_volume_days),
        textfont=dict(size=high_volume_days['MarkerSize'], color='blue'),
        textposition="middle center",
        name='High Volume (Z > 2)'
    ), row=1, col=1)
    
    # Highlight recurring days
    for day, count in recurring_days.items():
        recurring_dates = high_volume_days[high_volume_days['MonthDay'] == day].index
        fig.add_trace(go.Scatter(
            x=recurring_dates,
            y=df['Adj Close'][recurring_dates],
            mode='markers',
            marker=dict(
                size=15,
                color='gold',
            ...
            name=f'Recurring High Volume ({day})'
        ), row=1, col=1)
...
def main():
    ...
    recurring_days = identify_recurring_high_volume_days(high_volume_days) 
    ...
    date_ranges = {
        ...
        'Max': data.index[0]  # Add 'Max' period for the entire timeline
    }
    ...
    for period_name, start_date in date_ranges.items():
        ...
        image_file = plot_data(data_period, high_volume_days_period, recurring_days, ticker, period_name, start_date) 
        if image_file:
