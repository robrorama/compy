...
def download_data(ticker, period="max"):  # Downloading full historical data
    ...
    try:
        ...
        df = yf.download(ticker, period=period)  # Download full data if not found on disk
        if df.empty:
...
def main():
    ...
    period = sys.argv[2] if len(sys.argv) > 2 else "max"  # Download data for the entire available time frame
