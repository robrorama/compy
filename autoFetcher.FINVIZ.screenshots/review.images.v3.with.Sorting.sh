...
create_directories() {
    for dir in "RESULTS" "RESULTS/UPTREND" "RESULTS/DOWNTREND" "RESULTS/SIDEWAYS" "RESULTS/NOTSURE" "RESULTS/XREGENERATE"; do
        mkdir -p "$dir"
    done
}
...
move_image() {
    read -p "Enter u for UPTREND, d for DOWNTREND, s for SIDEWAYS, n for NOTSURE, x for XREGENERATE: " choice
    case "$choice" in
        u)
